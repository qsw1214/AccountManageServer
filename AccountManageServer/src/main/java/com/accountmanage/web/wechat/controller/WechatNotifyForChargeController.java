...
public class WechatNotifyForChargeController {

	@Resource
	private WechatNotifyForChargeService wechatNotifyForChargeService;
	...
	@RequestMapping(value = "/forcharge", method = RequestMethod.POST)
	public void toSendDispose(HttpServletRequest request,HttpServletResponse response){
		this.logger.info("接入微信订单充值接口的IP:" + request.getRemoteHost());
		try{
	...
	private void doDispose(HttpServletRequest request, HttpServletResponse response) throws Exception{		
		...
		if(returnSign){
			if("SUCCESS".equals(notify.getReturn_code()) && "SUCCESS".equals(notify.getResult_code())){			
				boolean isOk = wechatNotifyForChargeService.doWechatNotifyForCharge(notify);
				if(isOk){
